...
function RegisterPage() {
    ...
    const handleSubmit = () => {
        axios({
            ...
            .catch((err) => err.message==="Request failed with status code 409" ? alert('User already exists') : null);
    };

    const firstTab = (
        <div index={0} className="register__form__view">
            <Typography
                className="register__form__view__title"
                variant="h4"
                component="h4"
            >
                Agent Details
    ...
    const secondTab = (
        <div index={0} className="register__form__view">
            ...
            <TextField
                className="register__form__item"
                label="Password (Must have 8 or more characters)"
                required={true}
                value={pwd}
            ...
            <Paper className="register__form">
                <div className="register__form__tabspanel">
                    <div
                        ...
                        onClick={() => setTab(0)}
                        className="register__form__tab"
                    >
                        <ArrowBack color="primary" />
                    </div>
                    <div
                        style={{ visibility: tab === 1 ? "hidden" : "visible" }}
                        onClick={() => setTab(1)}
                        className="register__form__tab"
                    >
                ...
                <SwipeableViews axis={"x"} index={tab}>
                    {firstTab}
                    {secondTab}
                </SwipeableViews>
                ...
                <Button
                    disabled={
                        [company, agent, phone, email, pwd].includes("") ||
                        pwd.length < 8
                    }
                    variant="contained"
                    color="primary"
