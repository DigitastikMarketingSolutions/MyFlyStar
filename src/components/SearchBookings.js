...
function SearchBookings() {
    ...
    return (
        <div className="searchBookings">
            ...
            {bookings?.length
                ? bookings.map((item) => (
                      <SearchBookingsListItem key={item._id} item={item} />
                  ))
                : "No Bookings yet"}
                ...
                <div className="searchBookings__modal">
                    <div className="searchBookings__modal__header">
                        <span>
                            Airline:{" "}
                            {ticket.airline
                                ? state.flights[ticket?.airline][0]
                                : null}
                        </span>
                    ...
                    <div className="searchBookings__modal__header">
                        ...
                        <span>
                            Arrival:{" "}
                            {new Date(ticket?.arrival).toLocaleDateString(
                                "en-GB"
                            )}
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <u>
                                {new Date(ticket?.arrival)
                                    .toLocaleTimeString("en-US", {
                                        hour12: false,
                                    })
                                    .slice(0, 5)}
                        ...
                        <span>
                            Total Price(without markup): {booking?.realPrice}
                        </span>
                    </div>
                    <div className="searchBookings__modal__header">
                        ...
                        {booking?.adults?.map((i, idx) => {
                            return (
                                <div
                                    key={idx}
                                    className="searchBookings__modal__header__row"
                                >
                                    <span>{i}</span>
                                    <span>Adult</span>
                                </div>
